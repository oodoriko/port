...
async fn test_backtest2() {
    ...
    // Assert final value
    assert!(
        (final_value - 68080.45).abs() < 0.01,
        "Final value mismatch: expected 68080.45, got {:.2}",
        final_value
    );

    // Assert total return
    assert!(
        (total_return - (-31.92)).abs() < 0.01,
        "Total return mismatch: expected -31.92%, got {:.2}%",
        total_return
    );
    ...
    // Assert min value
    assert!(
        (min_equity - 68080.45).abs() < 0.01,
        "Min value mismatch: expected 68080.45, got {:.2}",
        min_equity
    );
